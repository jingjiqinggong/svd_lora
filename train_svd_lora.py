...
from utils.dataset11 import SingleVideoDataset
...
def my_save_lora(accelerator, unet,output_dir, global_step,save_state=False):
    save_path = os.path.join(output_dir, f"checkpoint-{global_step}")
    if save_state:
        accelerator.save_state(save_path)   #model.safetensors   optimizer.bin  random_states_0.pkl   scaler.pt  scheduler.bin
...
def my_draw_loss(loss_list, global_step, output_dir):
    x = list(range(1, global_step + 1))
    plt.figure()
    plt.plot(x, loss_list, color = "limegreen",linestyle="dotted",marker = "o")
    plt.xticks(x)
...
def get_train_dataset(dataset_types, train_data, tokenizer):
    ...
    dataset_cls = [SingleVideoDataset]
...
def load_primary_models(pretrained_model_path, huggingface_cache_dir, lora_path = None, eval=False):
    if eval:
        pipeline = StableVideoDiffusionPipeline.from_pretrained(pretrained_model_path, cache_dir= huggingface_cache_dir,local_files_only=True,
                                                dtype=torch.float16, variant='fp16')
    else:
        pipeline = StableVideoDiffusionPipeline.from_pretrained(pretrained_model_path, cache_dir= huggingface_cache_dir,local_files_only=True)
    bind_all_methods(pipeline, LoraLoaderMixin) # 将PeftAdapterMixin的方法绑定到UNetSpatioTemporalConditionModel 对象
    bind_all_methods(pipeline.unet,PeftAdapterMixin)  
    if eval and lora_path is not None:
        pipeline.text_encoder = pipeline.image_encoder  # 为了让load_lora_into_text_encoder成功运行
...
#         path, unet=unet_out).to(dtype=torch.float32)
...
def finetune_unet(pipeline, batch, use_offset_noise,
    ...
    c_noise = sigma.log() / 4           # 类比 stable diffusion 的 t
    loss_weight = (sigma ** 2 + 1) / sigma ** 2

    noisy_latents = latents + torch.randn_like(latents) * sigma
    input_latents = c_in * noisy_latents
    input_latents = torch.cat([input_latents, image_latents], dim=2)    # 视频加噪后，和第一帧的视频，在 channel维度concat
    if motion_mask:
...
def main(
    ...
    *_, config = inspect.getargvalues(inspect.currentframe())
    ...
    pipeline, tokenizer, feature_extractor, train_scheduler, vae_processor, text_encoder, vae, unet = load_primary_models(pretrained_model_path,huggingface_cache_dir)
    ...
    # my add
    # UNET_TARGET_MODULES = ["to_k", "to_q", "to_v", "to_out.0"]      # 不能写attn, to_out 不然会报错
    # UNET_TARGET_MODULES = ".*transformer_blocks.*(q|k|v|to_out\.0|to_out\.0|to_out\.1|)$"  # 注意 .*tranformer_blocks 包含了 temporal_trans...
    UNET_TARGET_MODULES = unet_lora_target_modules
    # UNET_TARGET_MODULES = ["to_k", "to_q", "to_v", "to_out.0","ff.net.0.proj"] 
    ...
    # my modify: len(dataloader)=1  每次enumerate取出的帧不一样, 不能cache
    # Latents caching
    ...
    cast_to_gpu_and_type([text_encoder,vae], accelerator.device, weight_dtype)
    ...
    for name, p in unet.named_parameters():
        if "lora" in name:
            weights_dict[name] = p.clone().detach().cpu()
    for epoch in range(first_epoch, num_train_epochs):
        train_loss = 0.0
        for step, batch in enumerate(train_dataloader):
            # Skip steps until we reach the resumed step
            if save_train_sample:
                video = rearrange(batch["pixel_values"], "b f c h w -> b f h w c")[0].to("cpu")
                os.makedirs(os.path.join(output_dir, "train_data"),exist_ok=True)
                imageio.mimwrite(os.path.join(output_dir, "train_data", "%s.mp4" % global_step), video, fps=7)
                global_step += 1
                continue
            ...
            with accelerator.accumulate(unet) ,accelerator.accumulate(text_encoder):
                ...
                # w1 = unet.down_blocks[0].attentions[0].transformer_blocks[0].attn1.to_q.lora_A.default.weight.data.detach().clone().cpu()
                # weights_his.append(w1)
            ...
            if accelerator.sync_gradients:
                ...
                my_draw_loss(train_loss_list, global_step, output_dir)
                train_loss = 0.0
                ...
                if global_step % checkpointing_steps == 0:
                    ...
                    # )
                    # my add
                    my_save_lora(accelerator,unet,output_dir, global_step)
    ...
    with open(os.path.join(output_dir, "loss.txt"),"w") as f:
        f.write("global step: %s\n" % global_step)
        f.write("global loss\n")
        f.write(str(train_loss_list))
        f.write("\n weight dis \n")
    for name, p in unet.named_parameters():
        if "lora" in name:
            t0 = weights_dict[name]
            t1 = p.clone().detach().cpu() 
            weights_dict[name] = (t0 - t1).abs().mean().item()
    weights_list = sorted(weights_dict.items(), key=lambda item: item[1], reverse=True)
    with open(os.path.join(output_dir, "weights.txt"),"w") as f:
        f.write("weights abs mean between trained and original\n")
...
def eval(pipeline, vae_processor, validation_data, out_file, index, forward_t=25, preview=True):
    ...
    motion_mask = pipeline.unet.config.in_channels == 9 # 第一帧(4 channel)、x_t 4个channel、mask 1个channel  默认加载出来是8
    ...
    with torch.no_grad():
        if motion_mask:
            ...
            video_frames = pipeline(
                ...
                num_inference_steps=validation_data.num_inference_steps,
                fps=validation_data.fps,
                decode_chunk_size=validation_data.decode_chunk_size,    # 省显存的  跟效果没关系
                motion_bucket_id=validation_data.motion_bucket_id,  # 运动幅度
            ).frames[0]
...
def main_eval(
    ...
    **kwargs
    ...
    pipeline, tokenizer, feature_extractor, train_scheduler, vae_processor, text_encoder, vae, unet = load_primary_models(pretrained_model_path, huggingface_cache_dir, lora_path = lora_path, eval=True)
    ...
    now = datetime.datetime.now().strftime("%Y-%m-%dT%H-%M-%S")
    output_dir = os.path.join(output_dir, f"eval_{now}")
    iters = 3
    for example in eval_list:
        for t in range(iters):
            name, prompt = example
            out_file_dir = f"{output_dir}/{name.split('/')[-1].split('.')[0]}"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, default="./example/train_svd_single_lora.yaml")
